...
export default function InstructorDashboard() {
  ...
  const handleUploadLesson = async (e) => {
    ...
    if (!uploadForm.lessonTitle) {
      toast('Please enter lesson title', 'error');
      return;
    }
    
    if (!uploadForm.videoUrl) {
      toast('Please enter video URL', 'error');
      return;
    }
    ...
    try {
      ...
      const lessonData = {
        title: uploadForm.lessonTitle,
        videoUrl: uploadForm.videoUrl,
        type: 'video'
      };
      
      const res = await fetch(`/api/instructor/courses/${uploadForm.courseId}/lessons`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (data.success) {
        toast('Lesson uploaded successfully!', 'success');
        setShowUpload(false);
        setUploadForm({
          courseId: "",
          lessonTitle: "",
        ...
        toast(data.error || 'Failed to upload lesson', 'error');
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto mt-6 p-4">
      ...
      {/* ✅ CREATE COURSE MODAL - POPUP FORM */}
